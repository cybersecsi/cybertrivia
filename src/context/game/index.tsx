...
interface ProviderInterface {
  ...
  setQuestionResult: (question: {category: string, id: number}, result: 'correct' | 'incorrect') => void;
}

const GameContext = createContext<ProviderInterface | null>(null);

const initialGameBoard = (): IGameBoard => {
  const gameBoard: IGameBoard =  Object.entries(questions).reduce((acc: IGameBoard, value: [string, IBaseGameQuestion[]]) => {
...
const GameProvider = ({ children }: any): any => {
  ...
  const setQuestionResult = (question: {category: string, id: number}, result: 'correct' | 'incorrect') => {
