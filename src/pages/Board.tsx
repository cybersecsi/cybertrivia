...
const Board = () => {
  ...
  const punteggio: number = useMemo(() => {
    return Object.entries(gameBoard).reduce((acc: number, v: [string,IGameQuestionWithId[]]) => {
      const categoryPoints = v[1].filter((q: IGameQuestionWithId) => q.status === 'correct').map((q: IGameQuestionWithId) => q.points)
  ...
  return (
              ...
              return <>
                {question.status === 'pending' ? (         
                  <Link to={`/question/${question.id}`}>     
                    <div className={`trivia-card ${styleMapping[question.status]}`}>{question.points} <BsArrowRight /></div>
                  </Link>
                ): (
                  <div className={`trivia-card ${styleMapping[question.status]}`}>{question.points}</div>
                )}
              </>
            })}
          </div>
    ...
    <h1 className='title text-cyan-900 text-center font-semibold'>Punteggio Totale: {punteggio}</h1>
    <div>
